...
def create_web_router(*, settings: Settings) -> APIRouter:
    ...
    async def ideas_page(request: Request):
        ...
        return _render(
            ...
            page_description="Explore different angles from the topics you have brought in.",
            account=await store.get_credit_account(owner_id),
            entitlements=resolve_entitlements(owner_id, settings=settings),
            **snapshot,
        )
    ...
    async def plans_page(request: Request):
        ...
        return _render(
            ...
            page_kind="plans",
            page_title="Drafts",
            page_description="Your developed stories, with sources attached. Open a draft, add your voice and keep writing.",
            account=await store.get_credit_account(owner_id),
            entitlements=resolve_entitlements(owner_id, settings=settings),
            **snapshot,
        )
    ...
    async def published_page(request: Request):
        ...
        return _render(
            ...
            page_description="Take a finished draft to your channel. Publishing is not connected yet.",
            account=await store.get_credit_account(owner_id),
            entitlements=resolve_entitlements(owner_id, settings=settings),
            **snapshot,
        )
    ...
    async def search_page(request: Request):
        ...
        return _render(
            ...
            page_kind="search",
            page_title="Search",
            page_description="Find the topic or idea you want to come back to.",
            query=query,
            account=await store.get_credit_account(owner_id),
            entitlements=resolve_entitlements(owner_id, settings=settings),
            **snapshot,
