...
def create_web_router(*, settings: Settings) -> APIRouter:
    ...
    async def signals_page(request: Request):
        ...
        return _render(
            ...
            page_kind="signals",
            page_title="Inbox",
            page_description="Sources, notes, and questions that can begin or extend a research Chain.",
            account=await store.get_credit_account(owner_id),
            entitlements=resolve_entitlements(owner_id, settings=settings),
            **snapshot,
        )
    ...
    async def ideas_page(request: Request):
        ...
        return _render(
            ...
            page_kind="ideas",
            page_title="Chains",
            page_description="Research trails you can inspect, challenge, continue, and turn into finished work.",
            account=await store.get_credit_account(owner_id),
            entitlements=resolve_entitlements(owner_id, settings=settings),
            **snapshot,
        )
    ...
    async def plans_page(request: Request):
        ...
        return _render(
            ...
            page_kind="plans",
            page_title="Outputs",
            page_description="Briefs, reports, and factual scripts that keep their sources and decisions attached.",
            account=await store.get_credit_account(owner_id),
            entitlements=resolve_entitlements(owner_id, settings=settings),
            **snapshot,
        )
    ...
    async def published_page(request: Request):
        ...
        return _render(
            ...
            page_description="Reconnect live work to the Chain that produced it and follow what the response opens.",
            account=await store.get_credit_account(owner_id),
            entitlements=resolve_entitlements(owner_id, settings=settings),
            **snapshot,
        )
    ...
    async def search_page(request: Request):
        ...
        return _render(
            ...
            page_kind="search",
            page_title="Search",
            page_description="Search sources, Chains, open questions, and finished work.",
            query=query,
            account=await store.get_credit_account(owner_id),
            entitlements=resolve_entitlements(owner_id, settings=settings),
            **snapshot,
