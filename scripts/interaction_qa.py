...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        connections_tab = page.locator('[data-trail-tab="connections"]')
        connections_tab.focus()
        page.keyboard.press("ArrowLeft")
        expect(page.locator('[data-trail-tab="source"]')).to_be_focused()
        expect(page.locator('[data-trail-panel="source"]')).to_be_visible()
        page.keyboard.press("End")
        expect(page.locator('[data-trail-tab="script"]')).to_be_focused()
        expect(page.locator('[data-trail-panel="connections"]')).not_to_be_visible()
        page.keyboard.press("Home")
        page.locator('[data-trail-next="connections"]').click()
        expect(connections_tab).to_be_focused()
        checks.append("story tabs support arrows, Home, End and associated panels")
        page.locator('[data-trail-idea="beer"]').click()
        expect(page.locator('[data-trail-treatment="beer"] [data-trail-hook]')).to_contain_text("brewer")
        expect(page.locator('[data-trail-treatment="beer"] .trail-outline li')).to_have_count(3)
        expect(page.locator('[data-trail-tab="script"]')).to_be_focused()
        checks.append("choosing a source connection opens its matching script idea")
